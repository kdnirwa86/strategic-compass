...
export function TopNav() {
  return (
    <header className="h-14 border-b border-border bg-card/80 backdrop-blur-xl flex items-center px-4 gap-4 shrink-0">
      ...
      <nav className="flex items-center gap-1">
        <Link
          ...
          className="text-[12px] px-2.5 py-1.5 rounded-md hover:bg-secondary transition data-[status=active]:bg-primary/10 data-[status=active]:text-primary"
        >
          Intelligence Canvas
        </Link>
        <Link
      ...
      <div className="h-6 w-px bg-border mx-1" />
      <NavItem label="Category" value="Confectionery" />
      <NavItem label="Geography" value="India · Tier 1–2" />
      <NavItem label="Window" value="L90D · Forecast 24M" />
...
function NavItem({ label, value }: { label: string; value: string }) {
  return (
...
export function SectionHead({ icon: Icon, title, sub }: { icon: React.ComponentType<{ className?: string }>; title: string; sub?: string }) {
  return (
...
function ChipBtn({ icon: Icon, label, active }: { icon: React.ComponentType<{ className?: string }>; label: string; active?: boolean }) {
  return (
    <button className={`flex items-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-md border transition ${active ? "bg-primary/15 border-primary/40 text-primary" : "bg-card border-border hover:border-primary/40"}`}>
...
export function KpiCards() {
  const cards = [
    { label: "Total Opportunity Size", value: "$4.82B", delta: "+18.4%", sub: "Addressable whitespace · 24mo", accent: "var(--primary)", icon: Target, spark: [12, 18, 16, 24, 22, 30, 34, 42, 48, 56, 62, 71] },
    { label: "Category Growth", value: "+34.2%", delta: "+6.1 pts", sub: "YoY momentum vs. category", accent: "var(--accent)", icon: TrendingUp, spark: [20, 22, 21, 28, 32, 30, 38, 44, 41, 52, 58, 64] },
    { label: "Active Signals", value: "12,847", delta: "+2,193", sub: "TikTok · Amazon · Patents · 7d", accent: "var(--chart-5)", icon: Activity, spark: [40, 38, 46, 44, 52, 50, 60, 58, 66, 70, 78, 84] },
    { label: "Whitespace Score", value: "91 / 100", delta: "+9", sub: "Gut-friendly indulgence cluster", accent: "var(--warning)", icon: Sparkles, spark: [50, 54, 52, 60, 64, 62, 70, 74, 78, 82, 88, 91] },
  ];
  return (
    <section className="grid grid-cols-4 gap-3">
      {cards.map((c) => {
        ...
        return (
          <div key={c.label} className="relative rounded-xl border border-border bg-card p-4 overflow-hidden group hover:border-primary/40 hover:shadow-md transition">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                <div className="size-7 rounded-md grid place-items-center" style={{ background: `color-mix(in oklab, ${c.accent} 14%, transparent)`, color: c.accent }}>
                  <Icon className="size-3.5" />
                </div>
                <div className="text-[10px] mono uppercase tracking-widest text-muted-foreground">{c.label}</div>
              </div>
              <span className="flex items-center gap-0.5 text-[10px] mono px-1.5 py-0.5 rounded" style={{ background: `color-mix(in oklab, ${c.accent} 12%, transparent)`, color: c.accent }}>
                <ArrowUpRight className="size-3" /> {c.delta}
              </span>
            </div>
            <div className="mt-3 flex items-end justify-between gap-2">
              <div>
                <div className="text-[26px] font-semibold tracking-tight leading-none" style={{ color: c.accent }}>{c.value}</div>
    ...
    <section className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      <div className="relative h-[340px] grid-bg">
        <div className="absolute inset-0 p-6">
          ...
          <div className="absolute top-3 left-8 text-[10px] mono uppercase tracking-widest text-primary/80">Whitespace</div>
          <div className="absolute top-3 right-4 text-[10px] mono uppercase tracking-widest text-warning">Crowded growth</div>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="none" viewBox="0 0 100 100">
            <line x1="0" y1="100" x2="100" y2="0" stroke="oklch(0 0 0 / 0.08)" strokeDasharray="2 3" strokeWidth="0.3" />
          </svg>
          {opps.map((o, i) => (
            <div key={i} className="absolute -translate-x-1/2 -translate-y-1/2 group cursor-pointer" style={{ left: `${o.x}%`, top: `${100 - o.y}%` }}>
              <div
                className="rounded-full transition-transform group-hover:scale-110"
                style={{
                  width: o.r * 2,
                  height: o.r * 2,
                  background: `radial-gradient(circle at 30% 30%, color-mix(in oklab, ${o.color} 85%, white), color-mix(in oklab, ${o.color} 45%, transparent))`,
                  boxShadow: `0 6px 20px color-mix(in oklab, ${o.color} 25%, transparent)`,
                }}
              />
              <div className="absolute left-1/2 -translate-x-1/2 mt-1 text-[10px] whitespace-nowrap text-foreground/85 group-hover:text-foreground">
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-4">
      ...
      <div className="space-y-2">
        {trends.map((t) => (
          <div key={t.name} className="rounded-lg border border-border bg-background p-3 hover:border-primary/40 transition cursor-pointer">
      ...
      <div className="grid grid-cols-[200px_1fr] gap-4 items-center">
        <div className="relative size-[200px] mx-auto">
          <svg viewBox="-100 -100 200 200" className="absolute inset-0">
            {[30, 55, 80, 95].map((r) => (
              <circle key={r} r={r} fill="none" stroke="oklch(0 0 0 / 0.08)" />
            ))}
            {[0, 60, 120].map((a) => (
              <line key={a} x1={-100} y1={0} x2={100} y2={0} transform={`rotate(${a})`} stroke="oklch(0 0 0 / 0.06)" />
            ))}
            <defs>
              <linearGradient id="sweep" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="oklch(0.62 0.17 160 / 0.35)" />
                <stop offset="100%" stopColor="oklch(0.62 0.17 160 / 0)" />
              </linearGradient>
            </defs>
            ...
            {players.map((p) => {
              ...
              return (
                <g key={p.n}>
                  <circle cx={x} cy={y} r={4 + p.level} fill="oklch(0.62 0.17 160)" opacity={0.9} />
                  <text x={x + 6} y={y + 3} fontSize="7" fill="oklch(0.2 0.02 260)">{p.n}</text>
                </g>
              );
            })}
          </svg>
    ...
    <section>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div className="text-[13px] font-semibold">Top 3 Opportunity Cards</div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {cards.map((c) => (
          <article key={c.title} className="rounded-xl border border-border bg-card p-4 hover:border-primary/40 hover:shadow-md transition group">
            <div className="flex items-start justify-between">
            ...
            <button className="mt-3 w-full text-[11px] py-1.5 rounded-md border border-border bg-background group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition flex items-center justify-center gap-1">
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="text-[13px] font-semibold">Causal Intelligence Graph</div>
        <span className="text-[10px] text-muted-foreground ml-auto mono uppercase tracking-widest">why opportunities exist</span>
      </div>
      <div className="relative h-[180px]">
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
          <defs>
            <marker id="arr" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="5" markerHeight="5" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="oklch(0.62 0.17 160 / 0.8)" />
            </marker>
          </defs>
          {edges.map(([a, b], i) => (
            <line key={i} x1={pos[a].x} y1={pos[a].y} x2={pos[b].x} y2={pos[b].y}
              stroke="oklch(0.62 0.17 160 / 0.55)" strokeWidth="0.4" markerEnd="url(#arr)" />
          ))}
        </svg>
        {nodes.map((n) => (
          <div key={n.id} className="absolute -translate-x-1/2 -translate-y-1/2"
            style={{ left: `${n.x}%`, top: `${n.y}%` }}>
            <div className="px-2 py-1 rounded-md border border-primary/40 bg-card text-[10px] whitespace-nowrap shadow-sm">
              {n.label}
            </div>
          </div>
        ))}
...
export function LeftPanel() {
  const reasoning = [
    { t: "Search acceleration detected", sub: "“low sugar dessert” +312% MoM" },
    { t: "Social signal convergence confirmed", sub: "TikTok ↔ Instagram ↔ YT Shorts" },
    { t: "Competitive response remains fragmented", sub: "2 of 7 incumbents reacting" },
    { t: "Opportunity score increased to 82", sub: "+9 pts vs last sweep" },
    { t: "Whitespace window estimated at 18 months", sub: "Decay risk: low" },
  ];
  const agents = [
  ...
  return (
    <aside className="bg-sidebar flex flex-col min-h-0">
      ...
      <div className="px-3 pb-3">
        <div className="rounded-lg border border-border bg-card p-3 space-y-2">
          ...
          <div className="flex items-center gap-2 rounded-md bg-background border border-border px-2 py-1.5">
            ...
            <input placeholder="What whitespace is emerging?" className="bg-transparent outline-none text-[12px] flex-1 placeholder:text-muted-foreground" />
      ...
      <div className="px-3 flex flex-wrap gap-1.5 pb-3">
        {investigations.map((i) => (
          <button key={i} className="text-[11px] px-2 py-1 rounded-md border border-border bg-card hover:bg-secondary hover:border-primary/40 transition">
            {i}
          </button>
        ))}
      </div>
      ...
      <div className="px-3 pb-3 space-y-1.5">
        {agents.map((a) => (
          <div key={a.name} className="flex items-center justify-between text-[12px] px-2.5 py-1.5 rounded-md bg-card border border-border">
            <div className="flex items-center gap-2">
...
export function RightPanel() {
  return (
    <aside className="bg-sidebar overflow-y-auto scrollbar-thin">
      ...
      <div className="px-3 pb-3 space-y-1.5">
        {[
          ...
          <div key={r} className="flex gap-2 text-[12px]">
            <div className="size-5 rounded-md bg-primary/15 text-primary grid place-items-center mono text-[10px] shrink-0">{i + 1}</div>
      ...
      <div className="px-3 pb-3 space-y-2">
        {[
          ...
          <div key={p.d} className="rounded-lg border border-border bg-card p-2.5">
...
function Kv({ k, v }: { k: string; v: string }) {
  return (
    <div className="flex items-center justify-between text-[12px] px-2.5 py-1.5 rounded-md bg-card border border-border">
